...
Public Class ServerDirectorySearch
    ...
    Public Function isSearchFile() As Integer
        ...
        f_Domain = My.Settings.DomainName
        f_UserID = My.Settings.SrvUserID
        f_Password = My.Settings.SrvPass
    ...
    Public Function DirectorySeach() As Integer
        ...
        Try
            ...
            Dim discovery = False
            discovery = SearchSubDirectory(FolderPath)

            If discovery = True Then
                ret = FileCheck.fTrue
            Else
                ret = FileCheck.fFalse
            End If
    ...
    Public Function SearchSubDirectory(Path As String) As Boolean

        Dim ret As Boolean = False

        'カレントディレクトリ検索
        For Each currFileName As String In Directory.GetFiles(Path)
            'ファイル有無確認
            Dim fileName As String = IO.Path.GetFileName(currFileName)
        ...
        'サブディレクトリ検索
        'If ret = False Then
        '    For Each subFileDir As String In Directory.GetDirectories(Path)
        '        ret = SearchSubDirectory(subFileDir)
        '        If ret = True Then
        '            Exit For
        '        End If
        '
